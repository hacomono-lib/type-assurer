 ...
 * ```
 */
export const isDate = createGuard(
  (target: unknown): target is Date => target instanceof Date
)
...
export const assertDate: TypeAssertOf<IsDate> = createAssertion(
  isDate,
  (target) => `Expected value to be an Date, but got ${JSON.stringify(target)}.`
)
...
export const assertNotDate: InvertedTypeAssertOf<IsDate> = createAssertion(
  isNotDate,
  (target) => `Expected value to not be an Date, but got ${JSON.stringify(target)}.`
)
