 ...
 * ```
 */
export const isBoolean = createGuard(
  (target: unknown): target is boolean => typeof target === 'boolean'
)
...
export const assertBoolean: TypeAssertOf<IsBoolean> = createAssertion(
  isBoolean,
  (target) => `Expected value to be an boolean, but got ${JSON.stringify(target)}.`
)
...
export const assertNotBoolean: InvertedTypeAssertOf<IsBoolean> = createAssertion(
  isNotBoolean,
  (target) => `Expected value to not be an boolean, but got ${JSON.stringify(target)}.`
)
