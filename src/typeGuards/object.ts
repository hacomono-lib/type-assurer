 ...
 * ```
 */
export const isObject = createGuard(
  <T extends Record<string, unknown>>(target: unknown): target is T => {
...
export const assertObject: TypeAssertOf<IsObject> = createAssertion(
  isObject,
  (target) => `Expected value to be an Object, but got ${JSON.stringify(target)}.`
)
...
export const assertNotObject: InvertedTypeAssertOf<IsObject> = createAssertion(
  isNotObject,
  (target) => `Expected value to not be an Object, but got ${JSON.stringify(target)}.`
)
